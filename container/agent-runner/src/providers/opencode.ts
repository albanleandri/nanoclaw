...
function createCurrentMemoryLifecycle(
  ...
  return {
    openingInstructions(systemInstructions) {
      if (isResume || !memory?.enabled) return systemInstructions;
      const rendered = memory.render();
      if (!rendered) return systemInstructions;
      return systemInstructions ? `${rendered}\n\n${systemInstructions}` : rendered;
    },
    pushPrefix(justCompacted) {
      if (!justCompacted || !memory?.enabled) return '';
      const rendered = memory.render();
...
export class OpenCodeProvider implements AgentProvider {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    const memory = this.memorySessionHook
      ? createMemoryLifecycle(this.memorySessionHook, Boolean(input.continuation))
      : createCurrentMemoryLifecycle(this.options.memory, Boolean(input.continuation));
    ...
    async function* gen(): AsyncGenerator<ProviderEvent> {
      ...
      while (!aborted) {
        ...
        const { text, attachments, onTurnResult } = pending.shift()!;
