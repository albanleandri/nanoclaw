...
describe('OpenCodeProvider neutral memory', () => {
  it('starts normally when memory is disabled', () => {
    expect(() => new OpenCodeProvider().query({ prompt: 'hi', cwd: '/workspace' })).not.toThrow();
  });

  it('renders current neutral memory for a new session, not a resume', () => {
    let renders = 0;
