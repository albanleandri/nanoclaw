...
/**
 * LEGACY-COMPAT: the `schedule_task` protocol-tool shim still calls this
 * (openai-protocol-loop has no `ncl` path — see D4 in the port design). New
 * code calls insertTaskRow directly.
 */
export function insertTask(
  db: Database.Database,
  task: { id: string; processAfter: string; recurrence: string | null; content: string },
): void {
  insertTaskRow(db, {
    id: task.id,
    seriesId: task.id,
