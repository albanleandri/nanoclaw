...
import { grantScheduleAdmin } from '../../db/schedule-admin-grants.js';
...
describe('tasks CLI resource', () => {
  ...
  // Regression for D2 — the fork keeps cross-group task delegation behind
  // schedule-admin grants. An agent with no grant must never resolve another
  // group, even if the dispatcher were bypassed.
  it('rejects an ungranted foreign --group at the handler', async () => {
    const res = await dispatch(
      { id: 't1', command: 'tasks-list', args: { group: 'ag-stranger' } },
      { caller: 'agent', sessionId: 'sess-1', agentGroupId: 'ag-1', messagingGroupId: 'mg-1' },
    );
    expect(res.ok).toBe(false);
  });

  // The test above is satisfied by dispatch.ts's cli_scope=group arg check
  // before groupArg ever runs, so on its own it does not prove the handler-side
  // backstop exists. These three drop that gate (cli_scope=full) so
  // resolveTaskGroup is genuinely the thing under test. Deleting them would let
  // adaptation 1 be reverted to upstream's unconditional `return
  // ctx.agentGroupId` with the suite still green.
  describe('grant-aware groupArg with the dispatcher scope gate disabled', () => {
    beforeEach(() => {
      ensureContainerConfig('ag-1');
      updateContainerConfigScalars('ag-1', { cli_scope: 'full' });
    });

    it('still rejects an ungranted foreign --group', async () => {
      const res = await dispatch(
        { id: 'g1', command: 'tasks-list', args: { group: 'ag-2' } },
        agentCtx('ag-1', 'chat-1'),
      );
      expect(res.ok).toBe(false);
      if (!res.ok) expect(res.error.message).toContain('schedule owner not authorized: ag-2');
    });

    it('allows a granted foreign --group', async () => {
      grantScheduleAdmin('ag-1', 'ag-2');
      const res = await dispatch(
        { id: 'g2', command: 'tasks-list', args: { group: 'ag-2' } },
        agentCtx('ag-1', 'chat-1'),
      );
      ...
      const res = await dispatch(
        {
          id: 'g3',
          command: 'tasks-create',
          args: { prompt: 'x', name: 'delegated', process_after: '2999-01-01T00:00:00Z' },
        },
        agentCtx('ag-1', 'chat-1'),
      );
      expect(res.ok).toBe(true);
      if (!res.ok) return;
      expect((res.data as { agent_group_id: string }).agent_group_id).toBe('ag-2');
    });
  });
