...
function groupArg(args: Record<string, unknown>, ctx: CallerContext): string | undefined {
  if (ctx.caller === 'agent') {
    // D2: an agent may target another group's tasks only with a live
    // schedule-admin grant. dispatch.ts has already rejected an ungranted
    // --group before we get here; this second check is the fail-closed
    // backstop for any future caller path that skips the dispatcher.
    //
...
function selectedSessions(args: Record<string, unknown>, ctx: CallerContext): ScopedSession[] {
  ...
  // Host, unscoped: every active session. Task rows can live in a chat session
  // (legacy series created before the ncl tasks port) as well as in per-series
  // task sessions, so both are in scope. withInbound() skips sessions whose
  // inbound.db is absent.
