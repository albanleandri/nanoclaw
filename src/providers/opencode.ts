 ...
 *
 * OpenCode's `opencode serve` process stores files below the XDG data, state,
 * config, and cache roots. We pin all four to a per-session host directory
 * mounted at /opencode-xdg. The
...
registerProviderContainerConfig('opencode', (ctx) => {
  ...
  // The host and container `node` users can have different numeric UIDs.
  // This directory is dedicated to one isolated session, so make the mount
  // and its XDG children writable to the unprivileged container user.
  fs.chmodSync(opencodeDir, 0o777);
  for (const child of ['data', 'state', 'config', 'cache']) {
    const childDir = path.join(opencodeDir, child);
    fs.mkdirSync(childDir, { recursive: true });
    fs.chmodSync(childDir, 0o777);
  }
