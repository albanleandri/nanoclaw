...
-- Host-owned screen-market guided wizard state
...
-- Host-owned interactive questions. These render like ask_question cards,
-- but responses are consumed by host modules rather than routed to agents.
...
-- Host-owned durable long jobs. Agents request jobs through NanoClaw tools;
-- the host owns lifecycle, progress, cancellation, and delivery truth.
CREATE TABLE jobs (
  id                 TEXT PRIMARY KEY,
  type               TEXT NOT NULL,
  status             TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS messages_in (
  ...
  -- Correlates a host-created orchestration run without changing the
  -- adapter-provided message id. NULL for non-orchestrated inbound rows.
