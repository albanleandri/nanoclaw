...
describe('sendPoolMessage', () => {
  ...
  it('routes all messages from the same sender to the same bot', async () => {
    ...
    const totalCalls = createdApis.reduce((sum, api) => sum + (api.sendMessage.mock.calls.length as number), 0);
    expect(totalCalls).toBe(3);
    const botWithAll = createdApis.find((api) => api.sendMessage.mock.calls.length === 3);
...
describe('splitAtBoundary', () => {
  ...
  it('falls back to hard character split when no boundary exists in the window', () => {
    ...
    expect(splitAtBoundary(text, 4096)).toEqual(['a'.repeat(4096), 'a'.repeat(904)]);
  });
