...
The host keeps `.env` owner-only and does not mount it or a copy into agent containers. Current installs keep provider credentials in OneCLI Agent Vault; the host injects only the resolved per-provider runtime values.
