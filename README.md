...
- **Owned shared knowledge** — explicit per-group grants expose only selected shared resources. Pilot resources and non-owner grants are filesystem read-only; one approved owner receives write access only after classification, isolated OKF validation, and reconciliation approval.
