...
- **Provider-neutral shared to-dos** — every agent granted the shared `knowledge` resource manages the same Markdown list through host-serialized `ncl todos` operations; the file is mounted read-only in every agent, including the knowledge owner, so providers cannot create a second writer path. See [docs/todos.md](docs/todos.md)
...
- **Built-in Telegram adapters and bot-pool routing** — Telegram is included here, with pairing support, Markdown sanitization, dedicated Claude/Codex/Lumo bot identities, canonical human identity across those bots, and optional pool routing via explicit `bot_index`.
